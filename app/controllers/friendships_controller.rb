class FriendshipsController < ApplicationController
  ...
  def index
    ...
    @non_friends = User.where.not(id: approved_friendships.pluck(:friend_id) + [current_user.id]  +  pending_friendships.pluck(:friend_id)  + inverse_approved_friendships.pluck(:user_id) + inverse_pending_friendships.pluck(:friend_id) +  inverse_pending_friendships.pluck(:user_id))
  end 
  ...
  def destroy
    if current_user.friendships.find(params[:id])
